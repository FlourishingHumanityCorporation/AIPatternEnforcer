...
if (violations.length > 0) {
  if (isJson) {
    ...
    console.error('To fix: Move files to suggested locations or update the allowlist in');
    console.error('tools/enforcement/root-file-enforcement.js if they should be allowed.\n');
  }
  process.exit(1);
} else {
  if (!isQuiet && !isJson) {
    console.log('✅ Root directory is clean - no violations found');
  }
  if (isJson) {
    console.log(JSON.stringify({ violations: [] }, null, 2));
  }
