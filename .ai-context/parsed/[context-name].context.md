...
Generated: 2025-07-11T05:06:31.968Z

## Code Sections

### scripts/dev/ai-context-parser.js (lines 8-398)
...
function generateContextFile(contextName) {
  ...
  const relatedPatterns = Array.from(patterns.entries())
    .filter(([_, instances]) =>
      instances.some(inst => inst.context === contextName)
    );
  ...
  // Add references
  const contextReferences = Array.from(references.entries())
    .filter(([_, refs]) =>
      refs.some(ref => ref.context === contextName)
    );
...
function generateIDEHints() {
  ...
  const outputPath = path.join(process.cwd(), '.vscode', 'ai-annotations.code-snippets');
...
function generateSummaryReport() {
  ...
  Array.from(contexts.keys()).sort().forEach(contextName => {
  ...
  Array.from(patterns.keys()).sort().forEach(patternName => {
...
function main() {
  ...
  if (contexts.size > 0) {
    ...
    console.log('  1. Use "npm run ai:focus [context-name]" to focus on specific contexts');
...
Used in scripts/dev/ai-context-parser.js:10
...
- [file/decision] (referenced in scripts/dev/ai-context-parser.js:9)
