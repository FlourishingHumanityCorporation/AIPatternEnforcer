...
function validateTemplatePlaceholders(content, filePath) {
  const issues = [];
  const placeholders =
    content.match(HANDLEBARS_PATTERNS.valid.placeholder) || [];
...
function validateTemplateJSON(content, filePath) {
  ...
  try {
    ...
    if (
      config.type &&
      !["component", "feature", "hook", "api", "page"].includes(config.type)
    ) {
      issues.push({
        type: "config",
        severity: "medium",
...
// Hook logic
async function templateIntegrityValidator(input) {
  ...
  if (filePath.endsWith(".hbs")) {
    issues.push(...validateHandlebarsSyntax(actualContent, filePath));
    issues.push(...validateTemplatePlaceholders(actualContent, filePath));
  } else if (filePath.endsWith("template.json")) {
    issues.push(...validateTemplateJSON(actualContent, filePath));
  }
  ...
  if (criticalIssues.length > 0) {
    ...
    message += `File: ${FileAnalyzer.extractFileInfo(filePath).fileName}\n\n`;
    ...
    process.stdout.write(
      `⚠️ ${issues.length} template warnings in ${fileInfo.fileName}\n`,
    );
    return { allow: true };
  } else {
    // All good
