...
class CapabilityTracker {
    ...
    promoteToLevel(newLevel) {
        ...
        console.log(`🎉 Level Up! Promoted to Level ${newLevel}: ${this.levels[newLevel].name}`);
        console.log(`📋 New capabilities: ${this.levels[newLevel].capabilities.join(', ')}`);
    ...
    generateProgressReport() {
        ...
        console.log(`Current Level: ${status.currentLevel} - ${status.levelName}`);
        console.log(`Capabilities: ${status.capabilities.join(', ')}`);
        console.log(`Total Interactions: ${status.metrics.totalInteractions}`);
        console.log(`Success Rate: ${(status.metrics.successRate * 100).toFixed(1)}%`);
        
        if (status.nextLevel) {
            console.log(`\nProgress to Level ${status.nextLevel}:`);
            console.log(`Overall: ${status.progressToNext.percentage}%`);
        ...
        Object.entries(status.metrics.capabilityBreakdown).forEach(([cap, data]) => {
            ...
            console.log(`  ${cap}: ${data.successes}/${data.attempts} (${rate}%)`);
        });
...
if (require.main === module) {
    ...
    switch (command) {
        ...
        case 'record':
            ...
            console.log(`Recorded ${type} interaction: ${success ? 'success' : 'failure'}`);
            break;
        ...
        default:
            console.log('Usage:');
            console.log('  node capability-tracker.js status');
            console.log('  node capability-tracker.js record <type> <success> [details]');
            console.log('  node capability-tracker.js check-progression');
    }
}
