...
function checkContentComplexity(input, operationType) {
  ...
  const complexity = (content.match(/function|class|interface|type|const|let/g) || []).length;
...
function checkFile(args) {
  try {
    ...
    if (complexityCheck.tooComplex) {
      issues.push(`Too complex: ${complexityCheck.score}/${complexityCheck.limit} complexity score`);
    }
    ...
    console.log(JSON.stringify({ 
      status: 'ok',
      debug: `Good scope: ${scopeAnalysis.goodPatterns.length} positive patterns`
    }));
    ...
    console.log(JSON.stringify({ 
      status: 'ok',
      debug: `Scope limiter error: ${error.message}` 
    }));
  }
}
