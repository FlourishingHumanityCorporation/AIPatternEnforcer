...
class GuidedSetupWizard {
  ...
  async run() {
    ...
    logger.info(
      chalk.gray("Setting up your AI-enhanced development environment...\n"),
    );
    ...
    try {
      ...
      logger.info(
        "  • Run: npm run g:c MyComponent  (generate your first component)",
      );
      logger.info(
        "  • Run: npm run demo:generators   (explore all generators)",
      );
      logger.info("  • Check: QUICK-START.md         (2-minute orientation)\n");
    } catch (error) {
      logger.error(chalk.red("\n❌ Setup failed:"), error.message);
      logger.info(
        chalk.yellow("💾 Progress saved. Run the wizard again to continue."),
      );
      process.exit(1);
    }
  }
...
📁 Project Structure:
  ...
  └── shared/            # Common utilities`,
    ...
    logger.info(
      chalk.green(
        previews[stack] ||
          "Custom configuration will preserve your existing structure.",
      ),
    );
  }
  ...
  async enforcementLevel() {
    ...
    logger.info(
      chalk.gray(
        "ProjectTemplate can enforce coding standards automatically.\n",
      ),
    );
  ...
  async validateDependencies() {
    ...
    if (!fs.existsSync(packagePath)) {
      throw new Error(
        "package.json not found. Please run this wizard from a Node.js project root.",
      );
    }
    ...
    try {
      ...
      if (missingDeps.length > 0) {
        throw new Error(
          `Missing required dependencies: ${missingDeps.join(", ")}. Run 'npm install' first.`,
        );
      }
  ...
  async validateStructure() {
    ...
    if (missingDirs.length > 0) {
      ...
      logger.info(
        chalk.yellow(`Creating missing directories: ${missingDirs.join(", ")}`),
      );
    ...
    for (const file of essentialFiles) {
      if (!fs.existsSync(path.join(this.projectRoot, file))) {
        throw new Error(
          `Essential file ${file} not found. This may not be a valid Node.js project.`,
        );
      }
    }
  }
  ...
  async validateAIConfig() {
    ...
    if (!fs.existsSync(aiConfigPath)) {
      logger.info(
        chalk.yellow("AI configuration directory not found, creating..."),
      );
      try {
...
`;
    ...
    for (const [file, description] of Object.entries(aiFiles)) {
      ...
      if (!fs.existsSync(filePath)) {
        logger.info(
          chalk.yellow(
            `${description} not found, will be created during finalization`,
          ),
        );
      }
    }
  ...
  async validateEnforcement() {
    ...
    if (!fs.existsSync(enforcementPath)) {
      logger.info(
        chalk.yellow(
          "Enforcement tools not found, creating basic structure...",
        ),
      );
      try {
...
`;
        ...
        logger.info(
          chalk.yellow(
            `Warning: Could not create enforcement tools: ${error.message}`,
          ),
        );
    ...
    if (missingFiles > 0) {
      logger.info(
        chalk.yellow(
          `Note: ${missingFiles} enforcement tools not found - some features may be limited`,
        ),
      );
    }
  }
  ...
  async configureAITools() {
    ...
    try {
      ...
      if (
        this.config.aiTool === "cursor" ||
        this.config.aiTool === "multiple"
      ) {
...
`;
      ...
      if (
        ...
        const claudeSettings = {
          hooks: {
            post_edit:
              "npm run fix:docs:dry-run || echo 'Documentation fixes not available'",
          },
          context: {
...
`;
  ...
  async configureEnforcement() {
    ...
    const enforcementConfig = {
      ...
      checks: {
        ...
        bannedDocs: {
          ...
          level: 3,
          description:
            "Prevents creation of status/completion/summary documents",
        },
      },
      metrics: {
        enabled: true,
    ...
    if (this.config.customRules) {
      enforcementConfig.checks.fileNaming.enabled =
        this.config.customRules.includes("fileNaming");
      enforcementConfig.checks.imports.enabled =
        this.config.customRules.includes("imports");
      enforcementConfig.checks.documentation.enabled =
        this.config.customRules.includes("docs");

      if (this.config.blockingMode !== undefined) {
        enforcementConfig.checks.fileNaming.blockOnFailure =
          this.config.blockingMode;
        enforcementConfig.checks.imports.blockOnFailure =
          this.config.blockingMode;
        enforcementConfig.checks.documentation.blockOnFailure =
          this.config.blockingMode;
      }
    }
    ...
    const answers = await inquirer.prompt([
      {
        ...
        default: "custom",
        validate: (input) =>
          input.trim() ? true : "Stack specification required",
      },
      {
        type: "confirm",
        name: "customConfig",
  ...
  async expertAIConfiguration() {
    ...
    this.config.aiTool =
      answers.aiTools.length > 1 ? "multiple" : answers.aiTools[0] || "skip";
...
if (require.main === module) {
  ...
  if (options.help) {
    ...
    logger.info("Options:");
    logger.info(
      "  --expert-mode, --expert  Expert configuration with advanced options",
    );
    logger.info(
      "  --quick                  Quick setup (dependencies + hooks only)",
    );
    logger.info("  --help, -h              Show this help message");
    logger.info("\nExamples:");
    logger.info("  npm run setup:guided           # Standard guided setup");
    logger.info(
      "  npm run setup:expert           # Expert mode with advanced options",
    );
