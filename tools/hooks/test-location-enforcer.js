  ...
  for (const [category, config] of Object.entries(TEST_PATTERNS)) {
    if (config.pattern.test(filePath)) {
      ...
      if (config.testLocation === 'co-located') {
        ...
        const expectedSourcePath = path.resolve(path.dirname(filePath), path.basename(sourceFile));
        ...
        if (!fs.existsSync(expectedSourcePath)) {
          ...
          console.warn(`⚠️  Test file created without corresponding source: ${expectedSourcePath}`);
        }
      }
      return { allow: true };
    }
