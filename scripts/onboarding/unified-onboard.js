...
class UnifiedOnboardingWizard {
  ...
  async run() {
    ...
    logger.info(
      chalk.gray("Get to your first working component in <5 minutes\n"),
    );
  ...
  showSuccess() {
    ...
    if (this.generatedComponent) {
      ...
      logger.info(
        `  • components/${this.generatedComponent}/${this.generatedComponent}.tsx`,
      );
      logger.info(
        `  • components/${this.generatedComponent}/${this.generatedComponent}.test.tsx`,
      );
      logger.info(
        `  • components/${this.generatedComponent}/${this.generatedComponent}.stories.tsx`,
      );
      logger.info(
        `  • components/${this.generatedComponent}/${this.generatedComponent}.module.css\n`,
      );
    }
    ...
    logger.info(
      "  2. Run: npm run g:c NewComponent (generate more components)",
    );
    logger.info("  3. Run: npm run demo:generators  (explore all generators)");
    logger.info(
      "  4. Read: CLAUDE.md               (AI assistant guidelines)\n",
    );

    logger.info(
      chalk.gray(
        "💡 Tip: Use `npm run check:all` before commits to ensure quality",
      ),
    );
    logger.info(
      chalk.gray(
        "🔍 Validate setup: npm run validate:complete (comprehensive check)",
      ),
    );
  }
}
