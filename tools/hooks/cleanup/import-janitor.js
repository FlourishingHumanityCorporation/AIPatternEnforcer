...
function isJavaScriptFile(filePath) {
  return FileAnalyzer.isCodeFile(filePath);
}
...
function extractImports(content) {
  ...
  lines.forEach((line, index) => {
    ...
    // Side-effect imports
    const sideEffectMatch = trimmed.match(
      /^import\s+['"`]([^'"`]+)['"`](?:\s*;)?$/,
    );
    if (sideEffectMatch) {
      imports.push({
...
function findUsedImports(content, imports) {
  ...
  imports.forEach((imp) => {
    ...
    if (
      imp.defaultImport &&
      isIdentifierUsed(contentWithoutImports, imp.defaultImport)
    ) {
      isUsed = true;
    }
    ...
    if (
      imp.namespace &&
      isIdentifierUsed(contentWithoutImports, imp.namespace)
    ) {
      isUsed = true;
    }
...
function cleanupImports(content, filePath) {
  ...
  const needsCleanup =
    originalImportCount !== cleanedImportCount ||
  ...
  const nonImportLines = lines.filter(
    (_, index) => !imports.some((imp) => imp.line === index),
  );
...
// Hook logic
async function importJanitor(input) {
  ...
  const result = cleanupImports(content, targetPath);
  ...
  if (result.cleaned) {
    ...
    try {
      ...
      if (result.consolidatedCount > 0) {
        messages.push(
          `consolidated ${result.consolidatedCount} duplicate imports`,
        );
      }

      process.stdout.write(
        `🧹 Import cleanup: ${messages.join(", ")} in ${fileInfo.fileName}\n`,
      );
    } catch (writeError) {
      // Log error but don't fail the operation
      process.stderr.write(
        `Import Janitor warning: Could not write to ${targetPath}: ${writeError.message}\n`,
      );
    }
  }
