...
function detectPathType(filePath) {
  ...
  if (filePath.includes("CLAUDE.md") || filePath.includes("GOAL.md"))
    return "core-docs";
...
function getSuggestion(filePath, pathType) {
  ...
  return (
    ...
    "Create new files instead of modifying infrastructure"
  );
}
...
process.stdin.on("end", () => {
  try {
    ...
    const isHookDevelopment =
      filePath.includes("tools/hooks/") &&
      process.env.HOOK_DEVELOPMENT === "true";

    if (isHookDevelopment) {
      // Even in hook development mode, protect critical security code
      const content = toolInput.content || toolInput.new_string || "";
      const oldString = toolInput.old_string || "";

      // Block modifications to HOOK_DEVELOPMENT protection logic
      if (
        content.includes("HOOK_DEVELOPMENT") ||
        oldString.includes("HOOK_DEVELOPMENT")
      ) {
        console.error(
          `🔒 Security Code Protection Active\n` +
            `\n` +
            `❌ Cannot modify HOOK_DEVELOPMENT protection logic\n` +
            `📁 Protected: Core security implementation\n` +
            `\n` +
            `ℹ️ Even in hook development mode, security code is protected\n` +
            `✅ This prevents AI from disabling its own constraints\n` +
            `\n` +
            `💡 Only humans can modify environment protection logic`,
        );
        process.exit(2);
      }

      // Allow other hook development
      process.exit(0);
    }
    ...
    if (protection.protected) {
      ...
      require("fs").writeFileSync(
        "meta-project-guardian-block.json",
      ...
      const protectionReason =
        PROTECTION_MESSAGES[protection.type] ||
