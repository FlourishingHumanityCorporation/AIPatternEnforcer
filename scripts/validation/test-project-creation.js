...
if (fs.existsSync(testProjectPath)) {
  logger.info(chalk.yellow('Cleaning up existing test project...'));
...
async function runTest() {
  try {
    ...
    logger.info(chalk.green('✓ Project structure created'));
    ...
    try {
      ...
      logger.warn(chalk.yellow('⚠ Git initialization failed, continuing anyway'));
    }
    ...
    devProcess.on('close', (code) => {
      ...
      if (devStarted || code === null) {
        ...
        try {
          ...
          logger.info(chalk.green('✓ Component generation works'));

          // Step 5: Run tests
          logger.info(chalk.blue('\n🧪 Running tests...'));
          try {
            execSync('npm test', { cwd: testProjectPath, stdio: 'inherit' });
            logger.info(chalk.green('✓ Tests pass'));
          } catch (error) {
            logger.error(chalk.red('✗ Tests failed'));
          ...
          logger.info(chalk.green('  ✓ Project structure created'));
          logger.info(chalk.green('  ✓ Dependencies installed'));
          logger.info(chalk.green('  ✓ Dev server runs'));
          logger.info(chalk.green('  ✓ Component generation works'));
          logger.info(chalk.green('  ✓ Tests execute'));
          ...
          logger.error(chalk.red('Component generation or testing failed:'), error.message);
          process.exit(1);
        }
      } else {
